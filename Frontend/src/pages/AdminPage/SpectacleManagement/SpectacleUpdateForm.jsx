...
const SpectacleUpdateForm = ({ onClose, spectacle, onUpdate }) => {
    ...
    validSizes.forEach(size => {
      validColors.forEach(color => {
        const existingVariant = formData.variants.find(v => 
          v.framesize === size && v.color === color.name
        );
        newVariants.push({
          framesize: size,
          color: color.name,
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`http://localhost:5000/api/spectacle/update/${spectacle._id}`, payload, {
      ...
      {step === 1 ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="submit-btn">Update Spectacle</button>
          </div>
        </form>
        </div>
      )}

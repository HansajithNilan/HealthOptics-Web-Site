...
function MyAppointmentScreen() {
  ...
  const deleteAppointment = async (id) => {
    if (window.confirm("Are you sure you want to delete this appointment?")) {
      try {
        const response = await axios.delete(`http://localhost:5000/api/doctorappointment/deletedoctorappointment/${id}`);
        console.log(response.data);
        alert("Your appointment is deleted successfully.");
        window.location.reload();
      } catch (error) {
        console.log(error);
        alert("Error with deleting appointment.");
      }
    }
  };
        ...
        {appointments.map((appointment) => (
          <div className="fulldetailsapp" key={appointment._id}>
            <div ref={componentPDF}>
              <p>
                <span style={{ fontSize: "19px" }}>Patient Name: <b>{appointment.firstname} {appointment.lastname}</b></span><br />
                ...
                <span style={{ fontSize: "19px" }}>{appointment.email}<br /></span>
            ...
            <div className="updatedelectbtn">
              ...
              <button className="appdelect" onClick={() => deleteAppointment(appointment._id)}>Delete</button>
              <button className="reservationpdfbtn" onClick={generatePDF} style={{ marginLeft: "60px" }}>
                <i className="fa fa-download" aria-hidden="true"></i>
                <span style={{ fontSize: "10px", marginLeft: "10px" }}>Download PDF</span>
              </button>
            </div>
          </div>
        ))}
